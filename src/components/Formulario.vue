...
<p class="date">Fecha última revisión: {{format_date(date)}}</p>
...
export default{
    ...
    methods: {
        //getNew(): obten los datos de la API y metelos en las propiedades del componente (data)
        getNew(){
            ...
            .then (response =>{
                ...
                this.order = response.data.materia,
                console.log(this.order),
                this.date = response.data.seo_fecha_modificacion,
        ...
        //metodo para modificar el formato de una fecha (la que obtenemos de la API)
        format_date(value){
         if (value) {
